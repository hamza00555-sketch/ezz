// Local dish name → photo path map
export const dishImages: Record<string, string> = {
  'سوتو أيام': '/dishes/soto-ayam.png',
};

// External manifest cache — loaded once from NEXT_PUBLIC_DISH_MANIFEST_URL
let manifestCache: Record<string, string> | null = null;
let manifestPromise: Promise<void> | null = null;

export async function loadDishManifest(): Promise<void> {
  const url = process.env.NEXT_PUBLIC_DISH_MANIFEST_URL;
  if (!url || manifestCache !== null) return;
  if (manifestPromise) return manifestPromise;
  manifestPromise = fetch(url)
    .then((r) => r.json())
    .then((data: Record<string, string>) => { manifestCache = data; })
    .catch(() => { manifestCache = {}; });
...
// Checks local map then loaded manifest (sync — call loadDishManifest() first if manifest needed)
export function getDishImage(name: string): string | undefined {
  return dishImages[name] ?? manifestCache?.[name] ?? undefined;
}
